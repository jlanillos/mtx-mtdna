...
for ii in range(bamfiles.shape[0]):
    ...
    try:
        mt = pysam.view('-c',datadir + "/" + f + '.mt.cram','-q 10','-F 1536',mtchrom)
    ...
    if normalflag:
        try:
            mtnorm = pysam.view('-c',datadir + "/" + normalbam + '.mt.cram','-q 10','-F 1536',mtchrom)
    ...
    if normalflag:
        ...
        else: #@jlanillos: this is the actual option I'm changing
            countcall = ' '.join(["samtools mpileup --region", mtchrom, "--count-orphans --no-BAQ --min-MQ",str(minmapq), "--min-BQ", str(minbq),
                "--ignore-RG --excl-flags UNMAP,SECONDARY,QCFAIL,DUP --BCF --output-tags DP,AD,ADF,ADR --gap-frac 0.005 --tandem-qual 80 -L 10000000 -d 10000000 --open-prob 30 --fasta-ref",
                fasta, datadir + "/" + f + '.mt.cram', datadir + "/" + normalbam + '.mt.cram' + " | bcftools call --multiallelic-caller --ploidy", bcfploidy_genome,
